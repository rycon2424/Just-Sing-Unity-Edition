...
public class PitchTracker : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        mic.clip = Microphone.Start(null, true, 10, samplerate);
        mic.loop = true; // Set the AudioClip to loop
    ...
    void AnalyzeSound()
    {
        ...
        mic.GetOutputData(samples, 0); // fill array with samples
        int i = 0;
        float sum = 0f;
        ...
        // get sound spectrum
        mic.GetSpectrumData(spectrum, 0, FFTWindow.BlackmanHarris);
        float maxV = 0f;
        for (i = 0; i < binSize; i++)
        { // find max
